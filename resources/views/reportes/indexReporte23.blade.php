...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="centro">
            <div id="principal">
                <h1>Detalle de las revisiones de las flotas</h1>
 <!--               <div style="margin-left:16%; margin-top:30px">-->
                    <div>
                        <table id="automoviles" width="100%" cellspacing="0">
                            <thead>
                                <th>Codigo de la revisión</th>
                                <th>Fecha de entrada</th>
                                <th>Fecha de salida</th>
                                <th>Fecha próxima revisión</th>
                                <th>Falla</th>
                                <th>Nombre del taller</th>
                                <th>Código del automóvil</th>
                                <th>Sucursal del automóvil</th>                          
                            </thead>
                            <tbody>
                                @foreach ($revisiones as $r)
                                <tr>
                                    <td>{{$r->codigorevision}}</td>
                                    <td>{{$r->f_ent}}</td>
                                    <td>{{$r->f_sal}}</td>
                                    <td>{{$r->f_pro}}</td>
                                    <td>{{$r->falla}}</td>
                                    <td>{{$r->nombretaller}}</td>
                                    <td>{{$r->codigoaut}}</td>
                                    <td>{{$r->nomsucursal}}</td>
                                </tr>
                                @endforeach
